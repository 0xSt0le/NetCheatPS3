...
4. Outside the callback, the worker calls `ProcessContinue`.
5. DABR remains armed.
...
- A guarded `ThreadContinue(hit.ThreadId)` experiment returned `SN_S_OK`, but the callback histogram still only delivered `0x0035E7C0` for the hot writer test address. The default per-hit resume path was restored to `ProcessContinue` only.
...
- Do not call `ThreadContinue` from the normal logger path unless a new guarded experiment proves a concrete benefit. The tested thread-level resume experiment did not expose the missing hot writer PC.
...
## Hot-PC Step-Over Audit Status

The intended hot-PC step-over experiment is currently blocked. The SDK header and managed metadata confirm `SNPS3SetBreakPoint`, `SNPS3ClearBreakPoint`, and `TargetSpecificEventType.PPUExcTrap`, and the PPU trap payload matches the same thread/hardware-thread/PC/SP sequence used by DABR. However, the exact TMAPI-E.pdf breakpoint preconditions and complete trap/breakpoint event semantics have not been verified in this repo, and NetCheatPS3 does not yet have proven Set/ClearBreakPoint wrappers. Per the debugger-control safety rule, the experiment must remain documentation-only until those details are verified from the official reference.
